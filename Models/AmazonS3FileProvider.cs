    ...
    public class AmazonS3FileProvider : IAmazonS3FileProviderBase
    {
        ...
        public FileManagerResponse GetFiles(string path, bool showHiddenItems, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                if (path == "/")
                {
                    ...
                    cwd = CreateDirectoryContentInstance(true, path.Split("/")[path.Split("/").Length - 2], path, false, "Folder", 0, DateTime.Now, DateTime.Now, (response.CommonPrefixes.Count > 0) ? true : false, path.Substring(0, path.IndexOf(path.Split("/")[path.Split("/").Length - 2])));
            }
        ...
        public List<FileManagerDirectoryContent> GetFile(string path, bool showHiddenItems, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                { 
                    //cwd = CreateDirectoryContentInstance(false, path.Split("/")[path.Split("/").Length - 2], path, false, "Folder", 0, DateTime.Now, DateTime.Now, (response.CommonPrefixes.Count > 0) ? true : false, path.Substring(0, path.IndexOf(path.Split("/")[path.Split("/").Length - 2])));
                    //myList.Add(cwd);
                }
            }
            catch (Exception ex) { throw ex; }
